...
class AgentFrameworkProductManagementAgent:
    ...
    def __init__(self):
        ...
        self.delegate_agent_names = ['ProductAgent', 'MarketingAgent', 'RankerAgent']
        logger.info(
            'Initialized root agent=%s delegates=%s',
            self.agent.name,
            self.delegate_agent_names,
        )

    def _extract_response_authors(self, response: Any) -> list[str]:
        """Return unique author names emitted in an agent response."""
        authors: list[str] = []
    ...
    async def invoke(self, user_input: str, session_id: str) -> dict[str, Any]:
        ...
        """
        logger.info(
            'Agent invoke start session_id=%s root_agent=%s input_chars=%s',
            session_id,
            self.agent.name,
            len(user_input),
        )
        ...
        response_authors = self._extract_response_authors(response)
        logger.info(
            'Agent invoke complete session_id=%s root_agent=%s responding_agents=%s',
            session_id,
            self.agent.name,
            response_authors or ['unknown'],
        )

        parsed_response = self._get_agent_response(response.text)
        logger.info(
            'Agent invoke parsed session_id=%s is_task_complete=%s require_user_input=%s',
            session_id,
            parsed_response.get('is_task_complete'),
            parsed_response.get('require_user_input'),
        )
    ...
    async def stream(
        ...
        """
        logger.info(
            'Agent stream start session_id=%s root_agent=%s input_chars=%s',
            session_id,
            self.agent.name,
            len(user_input),
        )
        ...
        response_authors = self._extract_response_authors(final_response)
        logger.info(
            'Agent stream complete session_id=%s root_agent=%s responding_agents=%s chunk_count=%s',
            session_id,
            self.agent.name,
            response_authors or ['unknown'],
            len(chunks),
        )
        final_text = final_response.text or ''.join(chunks)
        if final_text:
            parsed_response = self._get_agent_response(final_text)
            logger.info(
                'Agent stream parsed session_id=%s is_task_complete=%s require_user_input=%s',
                session_id,
                parsed_response.get('is_task_complete'),
                parsed_response.get('require_user_input'),
            )
